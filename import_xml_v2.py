...
class NukeScriptGenerator:
    ...
    def __init__(self, colourspace: str = COLOURSPACE_LIST[0], local_work: bool = False):
    ...
    def create_nuke_script(self, shot: ParsedShot, shot_folder: str, 
        ...
        last_frame = shot.duration + 1000
        
        # Try to use template
        if template_path and os.path.isfile(template_path):
            self._create_from_template(
                script_path, template_path, shot, trimmed_paths,
                format_str, timestamp, last_frame
            )
        else:
            self._create_basic_script(script_path, shot, trimmed_paths, timestamp)
        
        return script_path
    
    def _build_relative_paths(self, shot: ParsedShot, shot_folder: str, 
    ...
    def _get_video_format(self, filepath: str) -> str:
    ...
    def _create_from_template(self, script_path: str, template_path: str,
                               shot: ParsedShot, trimmed_paths: List[str],
                               format_str: str, timestamp: str, last_frame: int):
        """Create Nuke script from template."""
        with open(template_path, "r", encoding="utf-8") as f:
            contents = f.read()
        
        contents = contents.replace("find&replace fps", "25")
        contents = contents.replace("find&replace duration", str(shot.duration))
        ...
        contents = contents.replace("find&replace read1_path", trimmed_paths[0] if trimmed_paths else "")
        contents = contents.replace("find&replace colourspace", self.colourspace)
        contents = contents.replace("find&replace shotname", shot.name)
        
        with open(script_path, "w", encoding="utf-8") as f:
            f.write(contents)
            
            # Add additional read nodes for multi-layer shots
            if len(trimmed_paths) > 1:
                offset = 280
                for i, path in enumerate(trimmed_paths[1:], start=2):
                    f.write(self._read_node(i, path, shot.duration, offset))
                    offset += 100
        
        print(f"[NukeGen] Created script: {script_path}")
    
    def _create_basic_script(self, script_path: str, shot: ParsedShot,
                              trimmed_paths: List[str], timestamp: str):
        """Create basic Nuke script without template."""
        with open(script_path, "w", encoding="utf-8") as f:
            f.write(f"# {shot.name} - Auto-generated\n")
            f.write(f"# Duration: {shot.duration}\n")
            f.write(f"# {timestamp}\n\n")
            
            for i, path in enumerate(trimmed_paths):
                f.write(self._read_node(i + 1, path, shot.duration, i * 100))
        
        print(f"[NukeGen] Created basic script: {script_path}")
    
    def _read_node(self, name: int, filepath: str, duration: int, offset: int) -> str:
...
class ImportWorker(QObject):
    ...
    def __init__(self, shots: List[ParsedShot], project_root: str, edit_name: str,
                 ...
                 add_to_db: bool, create_tasks: bool, template_path: str = None):
        super().__init__()
        self.shots = shots
        ...
        self.nuke_gen = NukeScriptGenerator(colourspace, local_work)
...
class XMLImportPage(QWidget):
    ...
    def _on_setup_single(self, shot: ParsedShot):
        ...
        try:
            nuke_gen = NukeScriptGenerator(self.colourspace, self.local_work)
            ...
            # Create Nuke script
            template = os.path.join(SCRIPT_DIR, "template_v06.nk")
            nuke_gen.create_nuke_script(shot, shot_folder, self.parser.project_root, template)
    ...
    def _on_make_single_shot(self) -> None:
        ...
        try:
            ...
            template_file_path = os.path.join(SCRIPT_DIR, "template_v06.nk")

            nuke_script_generator.create_nuke_script(
                shot=parsed_shot,
