...
class ImportXmlV2GeneratorTests(unittest.TestCase):
    ...
    def test_xml_parser_remaps_projects_unc_file_url_on_linux(self):
        ...
        with (
            mock.patch.object(module.path_mapping.platform, "system", return_value="Linux"),
            mock.patch.object(module.path_mapping.os.path, "exists", return_value=False),
        ):
        ...
        self.assertEqual(
            filepath,
            "/mnt/projects/PROJECTS/Job/plates/primary plate.mov",
        )

    def test_xml_parser_maps_expansion_b_file_url_on_linux(self):
        parser = module.XMLTimelineParser()

        with mock.patch.object(module.path_mapping.platform, "system", return_value="Linux"):
            filepath = parser._normalize_filepath(
                "file:///X:/Projects%20B/Job/plates/primary%20plate.mov"
            )

        self.assertEqual(
            filepath,
            "/Volumes/Expansion_B/Projects B/Job/plates/primary plate.mov",
        )

    def test_xml_parser_maps_expansion_b_file_url_on_windows(self):
        parser = module.XMLTimelineParser()

        with mock.patch.object(module.path_mapping.platform, "system", return_value="Windows"):
            filepath = parser._normalize_filepath(
                "file:///Volumes/Expansion_B/Projects%20B/Job/plates/primary.mov"
            )

        self.assertEqual(
            filepath,
            r"X:\Projects B\Job\plates\primary.mov",
        )
    ...
    def test_single_shot_dialog_remaps_clip_and_output_paths_before_accepting(self):
        ...
        mapped_clip_path = "/mnt/projects/PROJECTS/Job/plates/primary.mov"
        mapped_output_path = "/mnt/projects/PROJECTS/Job/VFX/timeline_A"
    ...
    def test_single_shot_dialog_maps_expansion_b_paths_before_accepting(self):
        dialog = module.SingleShotCreationDialog(add_to_db=False)
        source_clip_path = "X:/Projects B/Job/plates/primary.mov"
        source_output_path = "X:/Projects B/Job/VFX/timeline_A"
        mapped_clip_path = "/Volumes/Expansion_B/Projects B/Job/plates/primary.mov"
        mapped_output_path = "/Volumes/Expansion_B/Projects B/Job/VFX/timeline_A"

        try:
            dialog.clip_slots[0].path_line_edit.setText(source_clip_path)
            dialog.output_directory_path_line_edit.setText(source_output_path)
            dialog.shot_name_line_edit.setText("sho010")
